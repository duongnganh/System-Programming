...
void eval(char *cmdline)
{
	...
	if (!builtin_cmd(argv)) {
		...
		/* parent waits until foreground terminates */
		if (!bg){
			waitfg(pid);
		}
		else{
...
void do_bgfg(char **argv)
{
	...
	if (*argv[1] == '%'){ //jid
		if ((jid = atoi((char *)argv[1] + 1)) <= 0){ //handle errors
	...
	{
		if ((pid = atoi(argv[1])) <= 0){ //handle errors
			printf("%s: argument must be a PID or %%jobid\n", argv[0]);
			return;
		}
...
void sigchld_handler(int sig)
{
	...
	{
		if (WIFEXITED(status)) //child exitted normally
		{
			deletejob(jobs, pid);
		}
		else if (WIFSTOPPED(status)) //child stopped
		{
			if (getjobpid(jobs, pid)->state != ST){ //job is not stopped yet
...
void sigint_handler(int sig)
{
	...
	if (pid > 0){
		...
		{
			unix_error("kill (int) error: No such process");
			return;
		}
		printf("Job [%d] (%d) terminated by signal %d\n", jid, pid, sig);
		deletejob(jobs, pid);
...
void sigtstp_handler(int sig)
{
	...
	if (pid > 0){
		...
		{
			unix_error("kill (tstp) error: No such process");
			return;
		}
		struct job_t *this_job = getjobpid(jobs, pid);
		if (this_job->state != ST){ //job is not stopped yet
			this_job->state = ST;
			printf("Job [%d] (%d) stopped by signal %d\n", jid, pid, sig);
		}
	}
